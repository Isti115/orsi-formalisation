...
mutual
  ...
  predicateWithState (PCWP (concurrentProgram, predicate)) state =
    assert_total $ All (\ci => predicateWithState (CWP (ci, predicate)) state) concurrentProgram
    -- All (\ci => predicateWithState predicate (conditionalExecute state ci)) concurrentProgram
    -- All (predicateWithState predicate) (map (conditionalExecute state) concurrentProgram)
  --
  ...
  predicateToCondition (PCWP (concurrentProgram, predicate)) state =
    assert_total $ all (\ci => predicateToCondition (CWP (ci, predicate)) state) concurrentProgram
